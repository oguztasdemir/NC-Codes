translate={
        ...
        EN={
                ...
                update="Yazı Yazmak için buraya bas",

                updated="Yazı Yazmak için buraya bas; <bl>%s",
        ...
        TR={
                ...
                newMessage="Yeni bir mesajınız var! Gelen kutunuza gitmek ister misiniz?",

                intro="<bl>Paylaşım yapabilmek için <vp>[Durumunu Güncelle] <bl>butonuna basınız.",
                ...
                likedPeople="%s kişi bunu beğendi.",

                likeNotice="<j>• <v>%s <bl>senin paylaşımını beğendi.",
...
function friendlist(p)
        ...
        ui.addTextArea(20,"<bl>Arkadaşlarınıza daha rahat ulaşmak için üzerine tıklayın.\n"..table.concat(online,"\n").."\n"..table.concat(offline,"\n"),p,50,67,250,nil,0x31474F,0x27373F,1,true)
...
function eventTextAreaCallback(id,p,cb)
        ...
        if cb=="add_friend" then
                ...
                ui.addPopup(6,1,database[database[p].sendFriend]["sendFriend"].." adlı kullanıcı size arkadaşlık isteği gönderdi. Kabul ediyormusunuz?",database[p]["sendFriend"],300,200)
        ...
        if cb:sub(1,5)=="yanit" and database[cb:sub(7)] then
                ...
                ui.addPopup(3,2,database[p]["sms"].yanit.." adlı kullanıcıya mesaj gönder!",p,325,175,210)
...
function eventPopupAnswer(id,p,ans)
        ...
        if id==0 then
                ...
                if database[fixPlayerNick(ans)] then
                        ...
                        ui.addPopup(1,2,database[p]["sms"].kime.." adlı kullanıcıya mesaj yolla!",p,175,150,210)

                else

                        ui.addTextArea(0,"<bl><p align='center'>Böyle bir kullanıcı bulunamadı.",p,325,375,200,20,0x31474F,0x27373F,1,true)
...
function openProfile(p,s)
        ...
        if database[p]["friends"][s] then
                ...
                bar=bar.."<vp><a href='event:add_friend'>[Arkadaş Ekle]</a> <j><a href='event:profilmes'>[Mesaj Gönder]</a> <r>[Dürt]"

        end

        local text=trans("gender")..":"..database[fixPlayerNick(s)]["gender"].."\nToplam Paylaşım: "..database[fixPlayerNick(s)]["total"].."\nProfil Puanı: "..database[fixPlayerNick(s)]["point"].."\n<r>Paylaşımlar\n"..table.concat(database[fixPlayerNick(s)]["profile"]["feed"],"\n")
