...
function eventTextAreaCallback(id, p, cmd)
	if cmd == "entrada" then
		ui.addTextArea(0, "<p align='center'>Çekiliş seçiliyor. Lütfen bekleyin...", p, 200, 370, 400, 20, 1, 0x00ff00, 0.8,false)
		entrada(p, #participante + 1)
	elseif cmd == "inicio" then
		ui.addPopup(0, 2, "<p align='center'>Seçilişi kaç kişinin kazanacağını buraya yaz.", p, 300, 200, 200, false)
...
function eventNewPlayer(p)
	if not inicio and not fim then
		ui.addTextArea(0, "<p align='center'><a href='event:entrada'>Lütfen bekleyin.", p, 200, 370, 400, 20, 1, 0xffff00, 0.8, false)
...
function eventLoop()
	if inicio then
		if fast > 0 then
			...
			inicio = false
			fim = true
			ui.addTextArea(-1, "<p align='center'><a href='event:novo'>Yeni Çekiliş", adm, 650, 370, 100, 20, 1, 0x0000ff, 0.8,false)
		end
	end
	if fim then
...
ui.addTextArea(0, "<p align='center'><a href='event:entrada'>Çekilişe katılmak için <VP>buraya<n> tıkla!", nil, 200, 370, 400, 20, 1, 0xffff00, 0.8, false)
...
tfm.exec.setUIMapName("Devilstrkpro#5475")
