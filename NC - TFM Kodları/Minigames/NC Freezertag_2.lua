...
function start_new_game()
	...
	output_msg(OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' it is the freezing mouse!')
	output_msg(OUTPUT_MESSAGE_COLOR .. 'The round will start in 10 seconds!')
	countdown = 10
end
...
tfm.exec.setUIMapName("(NC)Ice tag by Devilstrkpro")
	...
	if second_mark then
		...
		elseif countdown == 0 then
			countdown_msg(OUTPUT_MESSAGE_COLOR .. 'Street!')
			countdown = -1
			can_attack = true
		elseif countdown == 20 then
			output_msg(OUTPUT_MESSAGE_COLOR .. 'Starting a new round!')
			start_new_game()
		elseif countdown > 20 then
			countdown = countdown - 1
		end
	...
	if time_remaining <= 1 then
		...
		output_msg(OUTPUT_MESSAGE_COLOR .. 'Oh no! ' .. OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' failed to freeze all the mice!')
		output_msg(OUTPUT_MESSAGE_COLOR .. 'Starting a new round!')
		start_new_game()
	end
...
function check_alive()
	...
	if all_dead then
		...
		output_msg(OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' froze all the mice!')
		output_msg(OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' wins!')
		output_msg(OUTPUT_MESSAGE_COLOR .. 'The next round will start in 5 seconds')
...
function eventPlayerDied(name)
	if name == ice_mouse then
		...
		output_msg(OUTPUT_MESSAGE_COLOR .. '' .. OUTPUT_NAME_COLOR .. ice_mouse .. OUTPUT_MESSAGE_COLOR .. ' died!')
		output_msg(OUTPUT_MESSAGE_COLOR .. 'The next round will start in 5 seconds')
		set_new_game_countdown(5)
	else
		output_msg(OUTPUT_NAME_COLOR .. name .. OUTPUT_MESSAGE_COLOR .. ' was frozen!')
...
function search_and_rescue(rescuee)
	for id in pairs(ice_blocks) do
		if (math.abs(tfm.get.room.objectList[id].x - tfm.get.room.playerList[rescuee].x) <= RESCUE_RADIUS) and (math.abs(tfm.get.room.objectList[id].y - tfm.get.room.playerList[rescuee].y) <= RESCUE_RADIUS) then
			output_msg(OUTPUT_NAME_COLOR .. ice_blocks[id] .. OUTPUT_MESSAGE_COLOR .. ' has been thawed from ' .. OUTPUT_NAME_COLOR .. rescuee)
