...
local ball = {currentCoords = {x = 0, y = 0}, ID = 604, spawnCoords = {x = 600, y = 200}}
local banned = {Etho = true}
local blue = {}
local keys = {space = 90, left = 37, up = 90, right = 39, down = 90, a = 65, c = 67, d = 68, e = 69, q = 81, s = 90, h = 72, p = 80, w = 90}
local players = {}
  ...
  Random = function()
    ...
    for i = 1, #playerList, 1 do
      ...
      if i % 2 == 0 then
        ...
        tfm.exec.movePlayer(n, ball.spawnCoords.x - 300, ball.spawnCoords.y)
      else
        blue[n] = true
  ...
  Join = function(team, n)
    if players[n] then
      ...
      elseif team == blue then
        ...
        blue[n] = true
        tfm.exec.setNameColor(n, 0x0000FF)
        ui.addBotText("<VP>"..n.."<N> mavi takıma katıdı!", nil)
      end
      tfm.exec.killPlayer(n)
...
ui.addTextArea(3,"<font size='20'>"..score.red.."</a></p>",n,500.5,60,00,00,0x00001,0xFF0000,0.999999)
ui.addTextArea(4,"<font size='20'>"..score.blue.."</a></p>",n,700.5,60,00,00,0x00001,0x001CFF,0.999999)
...
function eventChatCommand(n, cmd)
  ...
  if admin[n] then
    ...
    elseif (arg[1] == "red" or arg[1] == "blue") and arg[2] then
      ...
      if players[pn] then
        ...
        ui.addBotText("This player does not exist.", n)
      end
 ...
 ui.addBotText("<VP>"..pn.."<N> art�k admin de�il!", n)
    elseif cmd == "overtime" then
      ...
      ui.addBotText("Zaman yak�nda bitecek!", nil)
    end
  end
  if players[n] then
    if cmd == "mort" or cmd == "afk" then
      if not spectators[n] then
        ...
        blue[n] = false
        tfm.exec.killPlayer(n)
        ui.addBotText("İzleyici modundasın! Sadece maçı izleyebilirsin!!!")
      end
