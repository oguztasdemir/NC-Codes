...
ui.addTextArea(3,"<font size='50'>"..score.red.."</a></p>",n,40,25,00,00,0.999999)
ui.addTextArea(4,"<font size='50'>"..score.blue.."</a></p>",n,740,25,00,00,0.999999)
...
function eventLoop(passed, left)
  ...
  if loopCountCharges == 0 and gameStart and loadedCharges and left > 2000 then
    ...
    charges.Load()
    ui.addBotText("Suçlamaları renerjik!", nil)
  end
  ...
  if banned[n] then
    ...
    if red[n] or blue[n] then
      ...
      elseif blue[n] and #blue < 5 then
        tasks[os.time() + 2000] = function() tfm.exec.respawnPlayer(n) ui.addBotText("<VP>"..n.." mavi tak�ma geri d�nd�!", nil) end;
      end
    end
  end
  if admin[n] then
...
function eventPlayerLeft(n)
  if red[n] then
    ui.addBotText(n.." Oyunu ve kırmızı takımı bıraktı. Kırmızı takımda şuan 1 kiii eksik bulunuyor!", nil)
  elseif blue[n] then
    ui.addBotText(n.." Oyunu ve mavi takımı bıraktı. Mavi takımda şuan  1 kiii eksik bulunuyor!", nil)
  end
end
  ...
  if players[n] then
    if cmd == "mort" or cmd == "afk" then
      if not spectators[n] then
        ...
        blue[n] = false
        tfm.exec.killPlayer(n)
        ui.addBotText("izleyici modundasın! Sadece ma.ı izleyebilirsin!!!")
      end
