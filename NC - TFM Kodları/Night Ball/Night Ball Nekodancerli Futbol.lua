...
local ball = {currentCoords = {x = 0, y = 0}, ID = 604, spawnCoords = {x = 400, y = 100}}
local banned = {Etho = true}
local blue = {}
local keys = {space = 90, left = 37, up = 9000, right = 39, down = 40, a = 65, c = 67, d = 68, e = 69, q = 81, s = 83, h = 72, p = 80, w = 940}
local players = {}
  ...
  Random = function()
    ...
    for i = 1, #playerList, 1 do
      ...
      if i % 2 == 0 then
        ...
        tfm.exec.movePlayer(n, ball.spawnCoords.x - 200, ball.spawnCoords.y + 250)
      else
        blue[n] = true
...
ui.addTextArea(3,"<font size='50'>"..score.red.."</a></p>",n,450.5,100,00,00,0x00001,0xFF0000,0.999999)
ui.addTextArea(4,"<font size='50'>"..score.blue.."</a></p>",n,550.5,100,00,00,0x00001,0x001CFF,0.999999)
tfm.exec.setUIMapName("<j>NIGHT BALL <bl>- NC'YE OZEL MAP")
end
  ...
  if player then
    ...
    if (key == keys.e or key == keys.space) and loadedCharges and not tfm.get.room.playerList[n].isDead and (player.pressKey < timestamp - 1000) then
      if player.boost < 150 then
        ...
        ui.addBotText("Çok fazla destek var.", n)
      end
...
function eventLoop(passed, left)
  ...
  if left < 5 then
    ...
    elseif gameStart then
      ...
      if not overTime then
        ...
        announceText("The "..winningTeam.." takım kazandı", 40)
        updateScore()
  ...
  if loopCountCharges == 0 and gameStart and loadedCharges and left > 2000 then
    ...
    charges.Load()
    ui.addBotText("Suçlamalarr renerjik!", nil)
  end
      ...
      if object then
        ...
        if not (object.x > (ball.spawnCoords.x - 10) and object.x < (ball.spawnCoords.x + 10)) then
          if (object.x == ball.currentCoords.x) and (object.y == ball.currentCoords.y) then
            ...
            ui.addBotText("Yeni top birkaç saniye içinde gelecek.", nil)
  ...
  if banned[n] then
    ...
    if red[n] or blue[n] then
      if red[n] and #red < 5 then
        tasks[os.time() + 2000] = function() tfm.exec.respawnPlayer(n) ui.addBotText("<VP>"..n.." kırmızı takıma geri döndü!", nil) end;
      elseif blue[n] and #blue < 5 then
        tasks[os.time() + 2000] = function() tfm.exec.respawnPlayer(n) ui.addBotText("<VP>"..n.." mavi takıma geri döndü!", nil) end;
      end
    end
  end
  if admin[n] then
...
function eventTextAreaCallback(id, n, cb)
  ...
  if cb == "soupBot" then
    ui.addTextArea(101, "Ne yapmak istiyorsun? <b><a href='event:bot_controls'>Modu değiştirin</a></b>, <b><a href='event:bot_particles'>Parçacıkların özelleştirildi.</a></b>, or <b><a href='event:bot_nothing'>nothing?", n, 6, 355, nil, 30, 0x324650, 0x89A7F5, 0.7, true)
  end
  ...
  if cb:sub(0, 7) == "release" then
    if players[n] then
      ...
      setKeys(n)
      ui.addBotText("Modun değiştirildi.", n)
    end
  end
...
function eventChatCommand(n, cmd)
  ...
  if admin[n] then
    ...
    elseif arg[1] == "ball" then
      ...
      for n in pairs(tfm.get.room.playerList) do
        if red[n] then
          tfm.exec.movePlayer(n, ball.spawnCoords.x - 300, ball.spawnCoords.y + 250)
        elseif blue[n] then
          tfm.exec.movePlayer(n, ball.spawnCoords.x + 300, ball.spawnCoords.y + 250)
        end
      end
    ...
    elseif (arg[1] == "red" or arg[1] == "blue") and arg[2] then
      ...
      if players[pn] then
        ...
        ui.addBotText("Böyle oyuncu odada bulunmuyor.", n)
      end
    ...
    elseif arg[1] == "lock" then
      if arg[2] then
        ...
        ui.addBotText("Oda <VP>"..num.." kişiye kilitlendi.<N> players.", n)
      else
        local num = 0
        for n in pairs(tfm.get.room.playerList) do
          num = num + 1
        ...
        tfm.exec.setRoomMaxPlayers(num)
        ui.addBotText("Oda <VP>"..num.." kişi kilidi kaldırıldı.<N> players.", n)
      end
  ...
  if players[n] then
    if cmd == "mort" or cmd == "afk" then
      if not spectators[n] then
        ...
        blue[n] = false
        tfm.exec.killPlayer(n)
        ui.addBotText("İzleyici modundasın! Sadece ma.ı izleyebilirsin!!!")
      end
